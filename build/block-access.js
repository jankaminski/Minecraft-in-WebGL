...
class BlockAccess {
    ...
    static getBlockCenterByWorldCoords(x, y, z) {
        let center = new Vec3((Math.trunc(x) / BLOCK_SIZE) + (BLOCK_SIZE / 2) * Math.sign(x), (Math.trunc(y) / BLOCK_SIZE) + (BLOCK_SIZE / 2) * Math.sign(y), (Math.trunc(z) / BLOCK_SIZE) + (BLOCK_SIZE / 2) * Math.sign(z));
        return center;
    }
...
export { CHUNK_WIDTH_IN_BLOCKS, CHUNK_HEIGHT_IN_BLOCKS, CHUNK_WIDTH, CHUNK_HEIGHT, TOTAL_BLOCKS_PER_CHUNK, BlockAccess };
